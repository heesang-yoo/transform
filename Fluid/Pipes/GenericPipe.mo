...
model GenericPipe
  ...
  // Heat Transfer Model
    ...
    HeatTransfer heatTransfer(
      ...
      final roughnesses=geometry.roughnesses,
      final surfaceAreas=geometry.surfaceAreas_nVtotal,
      final crossAreas=geometry.crossAreas,
      final dimensions=geometry.dimensions,
      final Ts_start=Ts_start) annotation (Placement(transformation(
          extent={{-8,-8},{8,8}},
          rotation=0,
          origin={-20,60})));
  ...
  TraceMassTransfer traceMassTransfer(
    ...
    surfaceAreas=geometry.surfaceAreas_nVtotal,
  ...
  parameter Boolean exposeState_b=false "=true, p is calculated at port_b else m_flow"
    annotation (Dialog(group="Model Structure", tab="Advanced"));
//   final parameter ModelStructure modelStructure= if exposeState_a == true and exposeState_b == true then ModelStructure.av_vb elseif exposeState_a == true and exposeState_b == false then ModelStructure.av_b
//       elseif exposeState_a == false and exposeState_b == true then ModelStructure.a_vb else ModelStructure.a_v_b
...
//     annotation (Dialog(tab="Advanced"), Evaluate=true);
  ...
  SI.Temperature[nV] Ts_wall(start=Ts_start)
  ...
  HeatAndMassTransfer.Interfaces.HeatPort_Flow heatPorts[nV] if use_HeatTransfer annotation (
      Placement(transformation(extent={{-10,50},{10,70}}), iconTransformation(extent={{-10,40},
            {10,60}})));
  HeatAndMassTransfer.Interfaces.MolePort_Flow massPorts[nV](each nC=Medium.nC) if use_TraceMassTransfer
...
equation
  ...
  // Heat Transfer connections
   connect(heatTransfer.heatPorts, heatPorts) annotation (Line(
       points={{-12,60},{-12,60},{0,60}},
       color={191,0,0},
       thickness=0.5));
  ...
  connect(traceMassTransfer.massPorts, massPorts) annotation (Line(
      points={{-52,60},{-40,60}},
      color={0,140,72},
      thickness=0.5));
  // Boundary Conditions
  ...
  if useLumpedPressure then
    ...
    if exposeState_a and exposeState_b then
      ...
      vsFM[2] =vs[iLumped:nV]*geometry.dlengths[iLumped:nV]/sum(geometry.dlengths[iLumped:
        nV]);
      Ts_wallFM[1] = Ts_wall[1];
      Ts_wallFM[2] =Ts_wall[nV];
    elseif exposeState_a and not exposeState_b then
    ...
    if exposeState_a and exposeState_b then
      ...
      port_b.p = mediums[nV].p;
      /* State Variables */
      statesFM[1:nFM+1] = mediums[1:nV].state;
      vsFM[1:nFM+1] = vs;
      Ts_wallFM[1:nFM+1] = Ts_wall[1:nV];
      /* Geometry Variables */
      crossAreasFM = geometry.crossAreas;
      dimensionsFM = geometry.dimensions;
      perimetersFM = geometry.perimeters;
      roughnessesFM = geometry.roughnesses;
       if nFM == 1 then
         dlengthsFM[1] = geometry.dlengths[1] + geometry.dlengths[2];
         dheightsFM[1] = geometry.dheights[1] + geometry.dheights[2];
       else
         dlengthsFM[1:nFM] = cat(1, {geometry.dlengths[1] + 0.5*geometry.dlengths[2]}, 0.5*(geometry.dlengths[2:nV-2] + geometry.dlengths[3:nV-1]), {0.5*geometry.dlengths[nV-1] + geometry.dlengths[nV]});
         dheightsFM[1:nFM] = cat(1, {geometry.dheights[1] + 0.5*geometry.dheights[2]}, 0.5*(geometry.dheights[2:nV-2] + geometry.dheights[3:nV-1]), {0.5*geometry.dheights[nV-1] + geometry.dheights[nV]});
       end if;
    elseif exposeState_a and not exposeState_b then
      /************************************************************************/
      ...
      /* State Variables */
      statesFM[1:nFM] = mediums[1:nV].state;
      vsFM[1:nFM] = vs;
      Ts_wallFM[1:nFM] = Ts_wall[1:nV];
      statesFM[nFM + 1] = state_b;
      vsFM[nFM + 1] = m_flows[nV + 1]/Medium.density(state_b)/geometry.crossAreas[nV];
      Ts_wallFM[nFM + 1] =Ts_wall[nV];
      /* Geometry Variables */
      crossAreasFM[1:nFM+1] = cat(1, geometry.crossAreas[1:nV], {geometry.crossAreas[nV]});
      dimensionsFM[1:nFM+1] = cat(1, geometry.dimensions[1:nV], {geometry.dimensions[nV]});
      perimetersFM[1:nFM+1] = cat(1, geometry.perimeters[1:nV], {geometry.perimeters[nV]});
      roughnessesFM[1:nFM+1] = cat(1, geometry.roughnesses[1:nV], {geometry.roughnesses[nV]});
      if nFM == 1 then
    ...
    elseif not exposeState_a and exposeState_b then
      ...
      vsFM[1] = m_flows[1]/Medium.density(state_a)/geometry.crossAreas[1];
      Ts_wallFM[1] = Ts_wall[1];
      statesFM[2:nFM+1] = mediums[1:nV].state;
      vsFM[2:nFM+1] = vs;
      Ts_wallFM[2:nFM+1] = Ts_wall[1:nV];
      /* Geometry Variables */
      crossAreasFM[1:nFM+1] = cat(1, {geometry.crossAreas[1]}, geometry.crossAreas[1:nV]);
      dimensionsFM[1:nFM+1] = cat(1, {geometry.dimensions[1]}, geometry.dimensions[1:nV]);
      perimetersFM[1:nFM+1] = cat(1, {geometry.perimeters[1]}, geometry.perimeters[1:nV]);
      roughnessesFM[1:nFM+1] = cat(1, {geometry.roughnesses[1]}, geometry.roughnesses[1:nV]);
      if nFM == 1 then
        dlengthsFM[1] = geometry.dlengths[1];
        dheightsFM[1] = geometry.dheights[1];
      else
    ...
    elseif not exposeState_a and not exposeState_b then
      ...
      vsFM[1] = m_flows[1]/Medium.density(state_a)/geometry.crossAreas[1];
      Ts_wallFM[1] = Ts_wall[1];
      statesFM[2:nFM] = mediums[1:nV].state;
      vsFM[2:nFM] = vs;
      Ts_wallFM[2:nFM] = Ts_wall[1:nV];
      statesFM[nFM + 1] = state_b;
      vsFM[nFM + 1] = m_flows[nV+1]/Medium.density(state_b)/geometry.crossAreas[nV];
      Ts_wallFM[nFM + 1] =Ts_wall[nV];
      /* Geometry Variables */
      crossAreasFM[1:nFM+1] = cat(1, {geometry.crossAreas[1]}, geometry.crossAreas[1:nV], {geometry.crossAreas[nV]});
      dimensionsFM[1:nFM+1] = cat(1, {geometry.dimensions[1]}, geometry.dimensions[1:nV], {geometry.dimensions[nV]});
      perimetersFM[1:nFM+1] = cat(1, {geometry.perimeters[1]}, geometry.perimeters[1:nV], {geometry.perimeters[nV]});
      roughnessesFM[1:nFM+1] = cat(1, {geometry.roughnesses[1]}, geometry.roughnesses[1:nV], {geometry.roughnesses[nV]});
      dlengthsFM[1:nFM] = cat(1, {0.5*geometry.dlengths[1]}, 0.5*(geometry.dlengths[1:nV-1] + geometry.dlengths[2:nV]), {0.5*geometry.dlengths[nV]});
      dheightsFM[1:nFM] = cat(1, {0.5*geometry.dheights[1]}, 0.5*(geometry.dheights[1:nV-1] + geometry.dheights[2:nV]), {0.5*geometry.dheights[nV]});
    else
      assert(false, "Unknown model structure");
    end if;
  end if;
...
<p><br>!!! Obsolete, sort of. This model is same as GenericPipe_MultiTransferSurface with the exception that it has one surface for heat transfer. To remove duplciations, etc. this model will be removed in favor of the other</p>
...
<p>The momentum balance is determined by the <b><span style=\"font-family: Courier New;\">FlowModel</span></b> component, which can be replaced with any model extended from <a href=\"modelica://Modelica.Fluid.Pipes.BaseClasses.FlowModels.PartialStaggeredFlowModel\">BaseClasses.FlowModels.PartialStaggeredFlowModel</a>. The default setting is <a href=\"modelica://Modelica.Fluid.Pipes.BaseClasses.FlowModels.DetailedPipeFlow\">DetailedPipeFlow</a>. </p>
<p>This considers </p>
<ul>
<li>pressure drop due to friction and other dissipative losses, and</li>
<li>gravity effects for non-horizontal devices.</li>
<li>variation of flow velocity along the flow path, which occur due to changes in the cross sectional area or the fluid density, provided that <span style=\"font-family: Courier New;\">flowModel.use_Ib_flows</span> is true. </li>
</ul>
<h4>Model Structure</h4>
<p>The momentum balances are formulated across the segment boundaries along the flow path according to the staggered grid approach. The configurable <b><span style=\"font-family: Courier New;\">modelStructure</span></b> determines the formulation of the boundary conditions at <span style=\"font-family: Courier New;\">port_a</span> and <span style=\"font-family: Courier New;\">port_b</span>. The options include (default: av_vb): </p>
<ul>
<li><span style=\"font-family: Courier New;\">av_vb</span>: Symmetric setting with nV-1 momentum balances between nV flow segments. The ports <span style=\"font-family: Courier New;\">port_a</span> and <span style=\"font-family: Courier New;\">port_b</span> expose the first and the last thermodynamic state, respectively. Connecting two or more flow devices therefore may result in high-index DAEs for the pressures of connected flow segments. </li>
<li><span style=\"font-family: Courier New;\">a_v_b</span>: Alternative symmetric setting with nV+1 momentum balances across nV flow segments. Half momentum balances are placed between <span style=\"font-family: Courier New;\">port_a</span> and the first flow segment as well as between the last flow segment and <span style=\"font-family: Courier New;\">port_b</span>. Connecting two or more flow devices therefore results in algebraic pressures at the ports. The specification of good start values for the port pressures is essential for the solution of large nonlinear equation systems.</li>
<li><span style=\"font-family: Courier New;\">av_b</span>: Asymmetric setting with nV momentum balances, one between nth volume and <span style=\"font-family: Courier New;\">port_b</span>, potential pressure state at <span style=\"font-family: Courier New;\">port_a</span></li>
<li><span style=\"font-family: Courier New;\">a_vb</span>: Asymmetric setting with nV momentum balance, one between first volume and <span style=\"font-family: Courier New;\">port_a</span>, potential pressure state at <span style=\"font-family: Courier New;\">port_b</span></li>
</ul>
<p>When connecting two components, e.g., two pipes, the momentum balance across the connection point reduces to </p>
<p><span style=\"font-family: Courier New;\">pipe1.port_b.p = pipe2.port_a.p</span></p>
...
<p><span style=\"font-family: MS Shell Dlg 2;\">Note that this generally leads to high-index DAEs for pressure states if this model are directly connected to each other, or generally to models with storage exposing a thermodynamic state through the port. This may not be valid if the model is connected to a model with non-differentiable pressure, like a Sources.Boundary_pT with prescribed jumping pressure. The </span><span style=\"font-family: Courier New;\">modelStructure</span><span style=\"font-family: MS Shell Dlg 2;\"> can be configured as appropriate in such situations, in order to place a momentum balance between a pressure state of the pipe and a non-differentiable boundary condition. </span></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">The default </span><span style=\"font-family: Courier New;\">modelStructure</span><span style=\"font-family: MS Shell Dlg 2;\"> is </span><span style=\"font-family: Courier New;\">av_vb</span><span style=\"font-family: MS Shell Dlg 2;\"> (see Advanced tab). The simplest possible alternative symmetric configuration, avoiding potential high-index DAEs at the cost of the potential introduction of nonlinear equation systems, is obtained with the setting </span><span style=\"font-family: Courier New;\">nV=1, modelStructure=a_v_b</span><span style=\"font-family: MS Shell Dlg 2;\">. Depending on the configured model structure, the first and the last segment, or the flow path length of the first and the last momentum balance, are of half size.</span></p>
<h4>Heat Transfer Structure</h4>
<p><span style=\"font-family: MS Shell Dlg 2;\">The </span><span style=\"font-family: Courier New;\">HeatTransfer</span><span style=\"font-family: MS Shell Dlg 2;\"> component specifies the source term </span><span style=\"font-family: Courier New;\">Qb_flows</span><span style=\"font-family: MS Shell Dlg 2;\"> of the energy balance. The default component uses a constant coefficient for the heat transfer between the bulk flow and the segment boundaries exposed through the </span><span style=\"font-family: Courier New;\">heatPorts</span><span style=\"font-family: MS Shell Dlg 2;\">. The </span><span style=\"font-family: Courier New;\">HeatTransfer</span><span style=\"font-family: MS Shell Dlg 2;\"> model is replaceable and can be exchanged with any model extended from <a href=\"TRANSFORM.Fluid.Pipes.BaseClasses.HeatTransfer.PartialFlowHeatTransfer\">TRANSFORM.Fluid.Pipes.BaseClasses.HeatTransfer.PartialFlowHeatTransfer</a>. </span></p>
